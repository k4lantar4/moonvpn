...
Revision ID: 55c4443161e1
Revises: 
Create Date: 2025-04-22 03:07:23.323850+03:30
...
def upgrade() -> None:
    ...
    # Tables depending on the first group
    ...
    # Tables depending on the second group (including nullable FK to client_accounts)
    op.create_table('orders',
    sa.Column('id', sa.Integer(), autoincrement=True, nullable=False),
    sa.Column('user_id', sa.BigInteger(), nullable=False),
    sa.Column('plan_id', sa.Integer(), nullable=False),
    sa.Column('location_name', sa.String(length=100), nullable=False),
    sa.Column('client_account_id', sa.Integer(), nullable=True), # Nullable FK to client_accounts
    ...
    # FK to client_accounts moved after client_accounts creation
    ...
    sa.Column('id', sa.BigInteger(), autoincrement=True, nullable=False),
    sa.Column('user_id', sa.BigInteger(), nullable=False),
    sa.Column('related_order_id', sa.Integer(), nullable=True), # Changed BigInteger to Integer to match orders.id
    sa.Column('amount', sa.DECIMAL(precision=10, scale=2), nullable=False),
    ...
    # Client Accounts needs all previous tables
    ...
    sa.ForeignKeyConstraint(['order_id'], ['orders.id'], name=op.f('fk_client_accounts_order_id_orders')),
    ...
    # Add FK constraint from orders to client_accounts after client_accounts is created
    op.create_foreign_key(
        op.f('fk_orders_client_account_id_client_accounts'),
        'orders', 'client_accounts',
        ['client_account_id'], ['id']
    )

    # Tables depending on client_accounts and others
...
def downgrade() -> None:
    ...
    op.drop_constraint(op.f('fk_orders_client_account_id_client_accounts'), 'orders', type_='foreignkey') # Drop added FK
